...
class Inrun:
    def __init__(self,ylengthstr=25,runangle=30.*2.*pi/360.,radius=30,flat=5,radius2=20,takeoffAngle=25.*2.*pi/360.,takeoffHeight=3):
        ...
        self.D=.4	#airresistance crossectional constant
        self.g=9.81	#gravity
    ...
    def rinnekulma(self,x):
        ...
        in radians
        """
        suora=self.ylengthstr-(1-cos(self.runangle))*self.radius #this is the straight section, after this comes tranny
        if tan(self.runangle)*x<suora:
            y=tan(self.runangle)
            return self.runangle
        elif x<=(suora/tan(self.runangle)+self.radius*sin(self.runangle)):
            return arcsin(((suora/tan(self.runangle)+self.radius*sin(self.runangle))-x)/self.radius)
        elif x<((suora/tan(self.runangle)+self.radius*sin(self.runangle))+self.flat):
            return 0
        elif x<((suora/tan(self.runangle)+self.radius*sin(self.runangle))+self.flat+self.radius2*sin(self.takeoffAngle)):
            return -arcsin((x-(suora/(tan(self.runangle))+self.radius*sin(self.runangle))-self.flat)/self.radius2)
        else:
    ...
    def takeoff2(self):
        ...
        """
        kode=1
        while self.rinnekulma(self.sx[kode,0])>-self.takeoffAngle and kode<self.steps:
            kode=kode+1
