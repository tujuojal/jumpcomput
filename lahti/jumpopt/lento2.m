...
beta=arvot(1);
p=arvot(2);
[Tkorkein,vxa,X,Y]=nousulento2(beta,p);
if (Tkorkein==0)
    Tkoko=0;
else
...
%vy^2)*(alastulo))=0
T=fzero(@(t)nokka+vx(t,vxa)/sqrt(vx(t,vxa).^2+vy(t,vxa).^2)*(alastulo)+zx(t,vxa)+X-1/tan(alfa)*(-tan(beta)*nokka-zy(t,vxa)-Y-vy(t,vxa)/sqrt(vx(t,vxa)^2+vy(t,vxa).^2)*(alastulo)),2);
t=(0:.01:T);
