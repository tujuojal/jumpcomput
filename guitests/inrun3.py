...
class Inrun:
	def __init__(self,ylengthstr=25,runangle=30.*2.*pi/360.,radius=20,flat=5,takeoffAngle=25.*2.*pi/360.,takeoffHeight=3):
	...
	def rinnekulma(self,x):
		...
		if tan(self.runangle)*x<self.ylengthstr:
		...
		elif x<=(self.ylengthstr/tan(self.runangle)+radius*sin(self.runangle)):
			...
			return 0
		elif x<((self.ylengthstr/tan(self.runangle)+self.radius*sin(self.runangle))+self.flat+self.radius*sin(self.takeoffAngle)):
			return -arcsin((x-(self.ylengthstr/(tan(self.runangle))+self.radius*sin(self.runangle))-self.flat)/self.radius)
		else:
...
##rest after
	...
	inr=Inrun(ylengthstr,runangle,radius,flat,takeoffAngle,takeoffHeight)
	inr.inrun()
	kode=inr.takeoff2()
