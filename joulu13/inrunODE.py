...
#this is the right hand side function for vector ode du/dt = f(u,t), where u=[vx,vy,sx,sy]
    def f(u,time):
        ...
        ret_2 = -g-sqrt(vy**2+vx**2)*vy*self.A + self.g*cos(rk)*cos(rk)+self.g*self.C*cos(rk)*sin(rk)+cos(rk)*(vx**2+vy**2)*self.invradius(sx)+sin(rk)*self.C*(vx**2+vy**2)*self.invradius(sx)
        ret_3 = vx
        ret_4 = vy
...
#this uses scipy.integrate to solve the above ode
    def ratkaise(self):
        self.steps=pylab.linspace(0,self.steps,endpoint=False)
        alkuarvot=[0,0,0,0] #this says startposition and speed are 0
        u = scipy.integrate.odeint(f,alkuarvot,self.steps)
        self.vx, self.vy, self.sx, self.sy = u
